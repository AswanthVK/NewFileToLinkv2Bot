...
async def force_sub(c, m):
    if Var.UPDATES_CHANNEL:
        try:
            chat = await c.get_chat_member(Var.UPDATES_CHANNEL, m.from_user.id)
            ...
            if m.from_user.id in Config.AUTH_USERS:
                return await m.reply_text(f"Make me admin in @{Var.UPDATES_CHANNEL}")

        except UsernameNotOccupied:
            logger.warning("The forcesub username was Incorrect. Please give the correct username.")
            if m.from_user.id in Var.OWNER_ID:
